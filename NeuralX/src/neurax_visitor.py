...
class Visitor(NeuralXVisitorBase):
    ...
    #
    #   csvAMatriz("archivo.csv", [col1, col2, ...])
    #       Lee las columnas indicadas (por índice) y devuelve una Matriz.
    #
    #   csvAVector("archivo.csv", col)
    #       Lee una columna (por índice) y devuelve una Lista de valores.
    #
    #   txtALista("archivo.txt")
    #       Lee un .txt con un número por línea y devuelve una Lista.
    #
    _FUNCIONES_NATIVAS = {
        "csvAMatriz", "csvAVector", "txtALista", "predecirSimple", "csvAMatrizEncoded"
    }

    def _llamada_nativa(self, nombre, args):
        # ── csvAMatriz("ruta.csv", [col1, col2, ...]) ────────────────
        if nombre == "csvAMatriz":
            if len(args) != 2:
                raise RuntimeError("csvAMatriz: necesita 2 argumentos (ruta, [columnas])")
            ruta = args[0]
            cols_arg = args[1]
            if isinstance(cols_arg, (Lista, Arreglo)):
                cols = [int(c) for c in cols_arg.a_lista_python()]
            elif isinstance(cols_arg, list):
                cols = [int(c) for c in cols_arg]
            else:
                raise RuntimeError("csvAMatriz: el segundo argumento debe ser una lista de índices")
            csv = ArchivoCSV(ruta)
            filas_mat = []
            for i in range(csv.total_filas()):
                fila_raw = csv._filas[i]
                filas_mat.append([fila_raw[c] for c in cols])
            return Matriz(filas_mat)

        # ── csvAVector("ruta.csv", col) ──────────────────────────────
        if nombre == "csvAVector":
            if len(args) != 2:
                raise RuntimeError("csvAVector: necesita 2 argumentos (ruta, indice_columna)")
            ruta = args[0]
            col  = int(args[1])
            csv  = ArchivoCSV(ruta)
            resultado = Lista()
            for fila in csv._filas:
                resultado.agregar(fila[col])
            return resultado

        # ── txtALista("ruta.txt") ────────────────────────────────────
        if nombre == "txtALista":
            if len(args) != 1:
                raise RuntimeError("txtALista: necesita 1 argumento (ruta)")
            ruta = args[0]
            resultado = Lista()
            with open(ruta, "r", encoding="utf-8") as f:
                for linea in f:
                    ...
                    if not linea:
                        continue
                    try:
                        val = float(linea) if "." in linea else int(linea)
                    except ValueError:
                        val = linea
                    resultado.agregar(val)
            return resultado

        # ── predecirSimple(modelo, [val1, val2, ...]) ────────────────
        if nombre == "predecirSimple":
            if len(args) != 2:
                raise RuntimeError("predecirSimple: necesita 2 argumentos (modelo, [valores])")
            modelo   = args[0]
            vals_arg = args[1]
            if isinstance(vals_arg, (Lista, Arreglo)):
                vals = vals_arg.a_lista_python()
            elif isinstance(vals_arg, list):
                vals = vals_arg
            else:
                raise RuntimeError("predecirSimple: el segundo argumento debe ser una lista de valores")
            fila = Lista()
            for v in vals:
                fila.agregar(v)
            entrada = Lista()
            entrada.agregar(fila)
            if not hasattr(modelo, "predecir"):
                raise RuntimeError("predecirSimple: se esperaba un modelo entrenado")
            resultado = modelo.predecir(entrada)
            if isinstance(resultado, (Lista, Arreglo)):
                datos = resultado.a_lista_python()
                return datos[0] if len(datos) == 1 else resultado
            return resultado


        # ── csvAMatrizEncoded("ruta.csv", [col1, col2, ...]) ─────────
        # Igual que csvAMatriz pero convierte texto a números
        # automáticamente (label encoding genérico).
        # Devuelve también el mapa de codificación para interpretarlo.
        if nombre == "csvAMatrizEncoded":
            if len(args) != 2:
                raise RuntimeError("csvAMatrizEncoded: necesita 2 argumentos (ruta, [columnas])")
            ruta = args[0]
            cols_arg = args[1]
            if isinstance(cols_arg, (Lista, Arreglo)):
                cols = [int(c) for c in cols_arg.a_lista_python()]
                ...
                raise RuntimeError("csvAMatrizEncoded: el segundo argumento debe ser una lista de índices")
            csv = ArchivoCSV(ruta)
            # Construir un mapa de encoding por columna
            # { col_idx: { valor_texto: numero } }
            encodings = {}
            for c in cols:
                muestra = csv._filas[0][c] if csv._filas else None
                if isinstance(muestra, str):
                    ...
                    for fila in csv._filas:
                        ...
                        if v not in vistos:
                            unicos.append(v)
                            vistos.add(v)
                    encodings[c] = {v: i for i, v in enumerate(unicos)}
            # Mostrar el mapa para que el usuario sepa qué número es qué
            if encodings:
                print("[NeuralX Encoding] Mapa de codificacion de texto:")
                for c, mapa in encodings.items():
                    print(f"  Columna {c}: {mapa}")
            # Construir la matriz aplicando encoding donde corresponda
            filas_mat = []
            for fila_raw in csv._filas:
                fila_enc = []
                for c in cols:
                    val = fila_raw[c]
                    if c in encodings:
                        fila_enc.append(float(encodings[c][val]))
                    else:
                        fila_enc.append(float(val))
                filas_mat.append(fila_enc)
            return Matriz(filas_mat)

        raise RuntimeError(f"Función nativa '{nombre}' no implementada")

    def visitLlamadaFuncion(self, ctx):
        nombre = ctx.IDENTIFICADOR().getText()

        # Interceptar funciones nativas antes de buscar en _funcs
        if nombre in self._FUNCIONES_NATIVAS:
            args = []
            if ctx.argumentos():
                args = [self.visit(e) for e in ctx.argumentos().expresion()]
            return self._llamada_nativa(nombre, args)
